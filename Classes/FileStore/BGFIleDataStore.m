...
{
    static NSString * mBGFileDataStorePayloadDirectory = nil;
    
	if(!mBGDataStoreDirectory)
    {
		mBGFileDataStorePayloadDirectory = BGDataStoreDirectory();
        mBGFileDataStorePayloadDirectory = [parent stringByAppendingPathComponent:@"FileDataStore" ];
        mBGFileDataStorePayloadDirectory = [parent stringByAppendingPathComponent:@"Payload" ];
	}
	
	return mBGFileDataStorePayloadDirectory;
}

extern inline NSString * BGFileDataStorePayloadFilePath(id key)
{
    return [BGFileDataStorePayloadDirectory() stringByAppendingPathComponent:BGDataStoreKey(key)];
}

@implementation BGFileDataStore
{
    NSCache * _cache;
...
- (id) init
{
    if(self = [super init])
    {
        ...
        // Setup the Data Store Cache Directory
        _fileManager = [NSFileManager defaultManager];
        
        NSError * error = nil;
        
        [_fileManager createDirectoryAtPath:BGDataStoreDirectory()
        ...
        [_fileManager createDirectoryAtPath:BGFileDataStorePayloadDirectory()
                withIntermediateDirectories:YES
                                 attributes:nil
                                      error:&error];
        ...
        [_fileManager createDirectoryAtPath:BGNanoDataStoreLargeDataDirectory()
                withIntermediateDirectories:YES
                                 attributes:nil
                                      error:&error];
...
#pragma mark - Wrappers for Data Store, used in multiple places
